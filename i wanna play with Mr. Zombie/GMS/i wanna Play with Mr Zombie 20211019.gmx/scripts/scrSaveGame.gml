...
{    
    ...
    //Added
    for (var i = 0; i < global.levelItemTotal; i++)
    {
        global.saveLevelItem[i] = global.levelItem[i];
    }
...
for (var i = 0; i < global.levelItemTotal; i++)
{
    ds_map_add(saveMap,"saveLevelItem["+string(i)+"]",global.saveLevelItem[i]);
}

//Added
if(savePosition){
    ds_map_add(saveMap, "dayNight", global.dayNight);
    //show_message("save true: " + string(ds_map_find_value(saveMap, "dayNight")));
}
else{
    ds_map_add(saveMap, "dayNight", global.saveDayNight);
    //show_message("save false: " + string(ds_map_find_value(saveMap, "dayNight")));
}
