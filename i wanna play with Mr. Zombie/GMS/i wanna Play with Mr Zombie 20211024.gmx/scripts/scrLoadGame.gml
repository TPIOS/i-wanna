...
if (loadFile)
{
    ...
    if (saveMap != -1)  //check if the save map loaded correctly
    {
        ...
        {
            global.saveLevelItem[i] = ds_map_find_value(saveMap,"saveLevelItem["+string(i)+"]");
        }
        
        global.saveGameClear = ds_map_find_value(saveMap,"saveGameClear");
        
        //Added
        global.dayNight = ds_map_find_value(saveMap, "dayNight");
        global.saveDayNight = global.dayNight;
...
for (var i = 0; i < global.levelItemTotal; i++)
{
    global.levelItem[i] = global.saveLevelItem[i];
}

//Added
if(!loadFile){
    global.dayNight = global.saveDayNight;
    //show_message("load false: " + string(global.dayNight));
}
